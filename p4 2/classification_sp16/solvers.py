...
class GradientDescentSolver(Solver):
    ...
    def get_updates_with_momentum(self, loss_tensor, param_vars):
        ...
        "*** YOUR CODE HERE ***"
        for i in range(len(param_vars)):
            vel = self.momentum*vel_vars[i] - self.learning_rate*grad_tensors[i]
            weight = param_vars[i] + vel
            updates.append((vel_vars[i], vel))
            updates.append((param_vars[i], weight))
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        for iter_ in range(self.iterations):
            ...
            train_loss = session.run(loss_tensor, feed_dict={placeholders[0]: train_data[0],
...
class StochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        for iter_ in range(self.iterations):
            "*** YOUR CODE HERE ***"
            train_input, train_target = train_generator.next()         # get the input and target data for training set
            train_loss = session.run(loss_tensor, feed_dict={placeholders[0]: train_input, placeholders[1]: train_target})

            session.run(update_ops, feed_dict={placeholders[0]: train_input, placeholders[1]: train_target})

            val_input, val_target = val_generator.next()    # get the input and target data for the validation set
            val_loss = session.run(loss_tensor, feed_dict={placeholders[0]: val_input, placeholders[1]: val_target})
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        for iter_ in range(self.iterations):
            ...
            train_loss = session.run(loss_tensor,
                                     feed_dict={placeholders[0]: train_input, placeholders[1]: train_target})
